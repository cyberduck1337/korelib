...
namespace korelib
{
    ...
    class StandardStreamWrapper final
    {
      public:
        ...
        StandardStreamWrapper(const std::vector<uint8_t>& bytes)
            requires StandardOutputStream<TStream> || StandardInputOutputStream<TStream>
        {
            std::copy(bytes.begin(), bytes.end(), std::ostream_iterator<uint8_t>(m_stream));
        }
      ...
      private:
        ...
        {
            KORELIB_VERIFY_THROW(isOpen(), RuntimeException, "Stream is not open!");

            const std::size_t readSize = sizeof(T);
            const std::size_t readPos = readPosition();

            KORELIB_VERIFY_THROW(!eof(), RuntimeException, fmt::format("stream EOF at {:#08x}! Read size was {}", readPosition(), readSize));
            KORELIB_VERIFY_THROW(readPos + readSize <= m_size, RuntimeException, fmt::format("Attempt to read {} bytes while {} is available", readSize, m_size - readPos));
